...
class ProtocolFeatures(object):
    ...
    def parse_from_supported(supported):
        rate_limit_error = ProtocolFeatures.maybe_parse_rate_limit_error(
            supported)
        shard_id, sharding_info = ProtocolFeatures.parse_sharding_info(
            supported)
    ...
    def maybe_parse_rate_limit_error(supported):
        ...
        if vals is not None:
            code_str = ProtocolFeatures.get_cql_extension_field(
                vals, "ERROR_CODE")
            return int(code_str)
    ...
    def parse_sharding_info(options):
        ...
        sharding_algorithm = options.get(
            'SCYLLA_SHARDING_ALGORITHM', [''])[0] or None
        sharding_ignore_msb = options.get(
            'SCYLLA_SHARDING_IGNORE_MSB', [''])[0] or None
        shard_aware_port = options.get(
            'SCYLLA_SHARD_AWARE_PORT', [''])[0] or None
        shard_aware_port_ssl = options.get(
            'SCYLLA_SHARD_AWARE_PORT_SSL', [''])[0] or None
        log.info("Parsing sharding info from message options %s", options)
